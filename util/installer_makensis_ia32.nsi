...
!define COMPANY_NAME "Project Dreamland"
!define PRODUCT_VERSION "${JUMP_VERSION}" 
!define APP_NAME "Area51 Launcher"
...
;Define UI settings
!ifdef WIN_PATHS
    !define MUI_UI_HEADERIMAGE_RIGHT "..\images\area51-icon.ico"
    !define MUI_ICON "..\images\area51-icon.ico"
    !define MUI_UNICON "..\images\area51-icon.ico"
!else
    !define MUI_UI_HEADERIMAGE_RIGHT "../images/area51-icon.ico"
    !define MUI_ICON "../images/area51-icon.ico"
    !define MUI_UNICON "../images/area51-icon.ico"
...
!define MUI_FINISHPAGE_RUN "$INSTDIR\Area51-Launcher.exe"
...
Section
    ...
    ;Add the files
    !ifdef WIN_PATHS
        File /r "..\dist\Area51-Launcher-win32-ia32\"
    !endif
...
Section
    ...
    CreateShortCut "$SMPROGRAMS\${APP_NAME}\${APP_NAME}.lnk" "$INSTDIR\Area51-Launcher.exe" "" "$INSTDIR\resources\area51-icon.ico" "" "" "" "${APP_NAME} ${JUMP_VERSION}"
    CreateShortCut "$SMPROGRAMS\${APP_NAME}\Uninstall ${APP_NAME}.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\resources\area51-icon.ico" "" "" "" "Uninstall ${APP_NAME}"
    ...
    WriteRegStr HKCU "${UNINSTALL_KEY}" "DisplayIcon" "$INSTDIR\resources\app\images\area51-icon.ico"
    WriteRegStr HKCU "${UNINSTALL_KEY}" "Publisher" "${COMPANY_NAME}"
    WriteRegStr HKCU "${UNINSTALL_KEY}" "UninstallString" "$INSTDIR\Uninstall.exe"
    WriteRegStr HKCU "${UNINSTALL_KEY}" "InstallString" "$INSTDIR"
    WriteRegStr HKCU "${UNINSTALL_KEY}" "URLInfoAbout" "${APP_URL}"
    WriteRegStr HKCU "${UNINSTALL_KEY}" "HelpLink" "https://github.com/ProjectDreamland/launcher"
...
Function finishpageaction
    CreateShortCut "$DESKTOP\${APP_NAME}.lnk" "$INSTDIR\Area51-Launcher" "" "$INSTDIR\resources\app\images\area51-icon.ico" "" "" "" "${APP_NAME} ${JUMP_VERSION}"
FunctionEnd
